...
try:
    from core.intelligence_client import (
        MerlinIntelligenceClient,
        SimpleIntelligenceClient,
    )
except ImportError:
...
class MerlinIntelligenceManager:
    ...
    def __init__(self, aas_hub_url: str = "http://localhost:8000"):
        ...
        if MerlinIntelligenceClient:
            ...
            logger.info(
                "🧙‍♂️ Merlin Intelligence Manager initialized with AAS integration"
            )
        else:
            logger.warning(
            ...
            async with self.client as client:
                ...
                awareness = {
                    ...
                    "resources": {
                        ...
                        "network_connected": context.get("network_status", {}).get(
                            "connected", False
                        ),
                    },
                    "cloud_services": self._extract_cloud_services(context),
                    "optimization_opportunities": context.get(
                ...
                logger.info(
                    f"🧙‍♂️ System awareness updated: health={awareness['system_health']:.2f}"
                )
                return awareness
    ...
    async def get_intelligent_recommendations(
        ...
        try:
            async with self.client as client:
                ...
                merlin_recommendations = (
                    await self._get_merlin_specific_recommendations()
                )
                recommendations.extend(merlin_recommendations)

                logger.info(
                    f"🧙‍♂️ Got {len(recommendations)} intelligent recommendations"
                )
        ...
        try:
            async with self.client as client:
                ...
                cost_recommendations = (
                    await client.get_cost_optimization_recommendations()
                )

                # Get model selection recommendations
                model_recommendations = (
                    await client.get_model_selection_recommendations()
                )
    ...
    async def request_intelligent_task_processing(
    ...
    async def discover_collaboration_opportunities(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.client as client:
                ...
                logger.info(
                    f"🧙‍♂️ Found {len(opportunities)} collaboration opportunities"
                )
    ...
    def _extract_resource_usage(
        self, context: Dict[str, Any], resource_name: str
    ) -> float:
    ...
    async def _get_merlin_specific_recommendations(self) -> List[str]:
        ...
        try:
            ...
            if current_spend > cost_optimization_manager.budget_limit * 0.8:
                recommendations.append(
                    "🧙‍♂️ High LLM costs detected - consider using cost optimization features"
                )
        except ImportError:
            pass
        ...
        try:
            ...
            recommendations.append(
                "🧙‍♂️ Enable predictive model selection for better performance"
            )
        except ImportError:
            pass
...
def get_merlin_intelligence_manager(
...
async def main():
    ...
    if "error" not in awareness:
        ...
        print(
            f"Optimization Opportunities: {len(awareness['optimization_opportunities'])}"
        )
    else:
    ...
    if "error" not in optimization:
        print(
            f"Optimization Strategy: {optimization.get('optimization_strategy', 'unknown')}"
        )
    ...
    for opp in opportunities:
        print(
            f"  {opp['capability']}: {opp['collaboration_potential']} plugins available"
        )
