...
class BagOfHolding:
    ...
    def execute(self, spell_type: str, item_name: str = "", content: str = ""):
        merlin_logger.info(
            f"Merlin reaching into the Bag of Holding: {spell_type} for {item_name}"
        )
        ...
        try:
            if spell_type == "store":
                ...
                with open(
                    os.path.join(self.storage_path, f"{item_name}.txt"), "w"
                ) as f:
                    f.write(content)
                return {
                    "output": f"Placed {item_name} into the Bag of Holding. It is safe within the void."
                }
                ...
                if os.path.exists(path):
                    with open(path, "r") as f:
                        data = f.read()
                    return {
                        "output": f"Pulled {item_name} from the Bag of Holding:\n{data}"
                    }
                ...
                return {
                    "output": f"The Bag of Holding contains: {', '.join([i.replace('.txt', '') for i in items])}"
                }
