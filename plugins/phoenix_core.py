...
class PhoenixCore:
    ...
    def execute(self, spell_type: str, service_name: str = ""):
        merlin_logger.info(f"Merlin invoking the Phoenix Core: {spell_type} on {service_name}")
        ...
        try:
            if spell_type == "ignite":
                ...
                return {"output": f"Ignited the Phoenix Flame for {service_name}! The service has been burned and reborn from its own ashes."}
                ...
                return {"output": "Phoenix Rejuvenation complete. The digital fog has been cleared, and I feel renewed."}

            elif spell_type == "death_and_rebirth":
                # Graceful full system restart command
                return {"output": "Preparing for full Death and Rebirth cycle. See you in the next life, Creator."}
