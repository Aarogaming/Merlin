        ...
        with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                for root, dirs, files in os.walk(chat_dir):
                    for file in files:
                        zipf.write(os.path.join(root, file), 
                                   os.path.relpath(os.path.join(root, file), os.path.join(chat_dir, '..')))
            ...
            if os.path.exists(log_dir):
                for root, dirs, files in os.walk(log_dir):
                    for file in files:
                        zipf.write(os.path.join(root, file), 
                                   os.path.relpath(os.path.join(root, file), os.path.join(log_dir, '..')))
...
def cleanup_old_backups(backup_dir: str = "backups", keep: int = 7):
    try:
        backups = sorted([os.path.join(backup_dir, f) for f in os.listdir(backup_dir) if f.startswith("merlin_backup_")])
        if len(backups) > keep:
