# MerlinIoTChat.py
# Minimal Python script for Merlin chat on IoT devices (e.g., Raspberry Pi)
import os
import requests
import time
...
print("Type /history to load chat history.")
while True:
    user_input = input("IoT Device (You): ")
    if user_input.strip().lower() == "exit":
        break
    if user_input.strip().lower() == "/history":
        try:
            resp = requests.get(
                HISTORY_URL + user_id, headers={"X-Merlin-Key": API_KEY}, timeout=10
            )
            if resp.ok:
